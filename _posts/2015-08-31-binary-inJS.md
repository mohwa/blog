...
- **Blob**
	...
	- Blob Object
		...
		- `Blob Object` 는 File 과 같은 `불변 객체`를 나타내며, `raw data` 이다.
...
- **JavaScript Typed Array**
	...
	- 정의
		...
		- `Typed Array` 는 `raw binary data` 에 접근하기 위한 방법을 제공한다.
			...
			- *즉 자바스크립트로 raw binary data 를 다루기 위해 사용한다.*

		- **유연성**과 **효율성**을 위해 `Typed Array` 는 <span style="color:#c11f1f">buffer</span> 와 <span style="color:#c11f1f">view</span> 로 나눠 구현되어있다.

			- buffer:

				- `ArrayBuffer` 는 고정된 크기의 `raw binary data` 를 나타내기 위해 사용된다.

				- `ArrayBuffer` 클래스 통해 생성된  <span style="color:#c11f1f">buffer</span> 는 `데이터 청크`를 나타내는 객체이다.
				...
				- <span style="color:#c11f1f">buffer</span> 는 <u>저장된 데이터를 접근하기 위한 방법을 제공하지 않는다.</u>

					- 즉 <span style="color:#c11f1f">buffer</span> 를 통해서는 데이터를 다룰 수 없다.
			...
			- view:
				...
				- DataView:
					...
					- DataView <span style="color:#c11f1f">view</span> 는 <span style="color:#c11f1f">buffer</span> 에 저장된 데이터로 부터 값을 읽고, 쓰기 위한 `low-level 인터페이스` 를 제공한다.(getter/setter API 제공)
				...
				- Typed Array Views:
					...
					- 위 DataView 를 상속한 아래 `클래스`들을 통해 <span style="color:#c11f1f">buffer</span> 에 저장된 데이터를 다룰 수 있게된다.
...
- **Little-Endian or Big-Endian**
	...
	- 적용 이유:
		...
		- 각 <u>**CPU**(Intel / Spac) **타입**에 따라 차이(**데이터 저장 방식에 대한 차이**)를 보이는 <span style="color:#c11f1f">byte order</span></u> 는, 동일한 시스템 안에서만 데이터를 주고 받는다면, <span style="color:#c11f1f">Endian</span> 에 대해 전혀 신경쓸 필요가 없지만, <u>이기종간에 데이터를 주고 받을 경우</u>, 서로간의 **저장 방식 차이**로 인해 전혀 엉뚱한 결과를 반환하게 된다.

	- 서로 다른 Endian 간의 데이터 통신 해결책:

		- 공통되는 Endian(<u>서로간에 정해진 <span style="color:#c11f1f">Endian</span> 규칙</u>)으로 변환 후, 데이타를 주고/받는 방법.
		...
		- 또 하나의 방법은 <span style="color:#c11f1f">byte order</span>(바이트 저장 순서) 를 신경쓸 필요가 없는, **데이터 타입**을 사용하는 것이다. <span style="color:#c11f1f">char</span> 타입은 **1byte** 의 크기를 가지기때문에, <span style="color:#c11f1f">byte order</span> 에 대해 전혀 신경쓸 필요가 없다. 예를 들면 12345678 을 int 형으로 보내는 대신 **문자열** "12345678" 로 변환시켜서 전송하면 되는 것이다.
...
- 예제 소스에서는 **NodeJS** 및 **Socket.IO** 와 관련된 내용은 최대한 배제 하였습니다.(특별히 포스트 내용과 관련없다고 판단된.)

	- **파일 업로드**

		- File 및 FileReader API 를 지원하는 브라우저를 통해 파일 업로드 기능을 만들 수 있다.

			- *IE(10/11 포함) 브라우저는 지원하지 않는다고 보면된다.*
	...
	- **이미지 효과**
		...
		- 서버에서 내려받은 ArrayBuffer(**이미지** 데이터) 로 <span style="color:#c11f1f">view</span>(uInt8Array) 를 생성 후, 버퍼에 저장된 데이터를 조작한다.
	...
	- **ArrayBuffer 로 내려받은 비디오 플레이**
		...
		- 서버에서 내려받은 ArrayBuffer(**영상** 데이터) 로 <span style="color:#c11f1f">view</span>(in Typed Array Views) 를 생성 후, 버퍼에 저장된 데이터를 조작한다.

			- **영상** 및 오디오 데이터의 경우, <span style="color:#c11f1f">브라우저 지원 여부</span> 및 <span style="color:#c11f1f">지원 포맷</span>에 대해 반드시 확인해봐야한다.

			- 아래 소스는 Chrome 브라우저에서 `*.mp4` 및 `*.webm` 포맷으로만 테스트하였습니다.
		...
		- Source Example
			...
			// 생성된 view를 통해 Blob Object 를 생성한다.
			// mp4 파일 가능
			var blob = new Blob([uInt8Array], {type: 'video/webm'});

			// 생성된 Blob Object를 참조하는 URL 를 생성한다.
			var url = URL.createObjectURL(blob);

			// 생성된 Blob 객체를 참조하는 URL을 video.src 에 할당 후 로드한다.
			video.src = url;
			```
			- Server Side(use nodeJS)
	...
	- **Chunk 방식으로 내려받은 비디오 플레이**
		...
		- 서버에서 Chunk 방식으로 내려받은 ArrayBuffer(**영상** 데이터) 로 <span style="color:#c11f1f">view</span>(in Typed Array Views) 를 생성 후, 버퍼에 저장된 데이터를 조작한다.

		- MediaSource API 를 통해 내려받은 영상 데이터를 조작할 수 있다.

		- **영상** 및 오디오 데이터의 경우, <span style="color:#c11f1f">브라우저 지원 여부</span> 및 <span style="color:#c11f1f">지원 포맷</span>에 대해 반드시 확인해봐야한다.

		- 아래 소스는 Chrome 브라우저에서 `*.webm`(vorbis 및 vp8 코덱) 포맷으로만 테스트하였습니다.
		...
		- Source Example
			...
			- Cliend Side(use JS)
				...
				```javascript
				// stream 객체에 socket 객체를 전달 후 해당 이벤트를 바인딩한다.
				ss(socket).on('onSocketMsg', function(data) {
					...
					if (type === 'resultChunkVideoData') {
						...
						// 생성된 MediaSource 객체를 참조하는 URL 를 생성한다.
						var url = URL.createObjectURL(ms);

						// 생성된 MediaSource 객체를 참조하는 URL을 video.src 에 할당 후 로드한다.
						video.src = url;

						// MediaSource 의 각 이벤트를 바인딩 시킨다.
