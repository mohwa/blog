...
- JS 는 **클래스 기반 언어**인 C or C++ 과 같이, 제어/반복 문 block 내부에 Local Context 를 갖지 않는다.
...
- <span style="color:#c11f1f">VO</span> in Eval Execution Context
  ...
  - <span style="color:#6298c1">eval</span> 함수에서는 Calling Context 라는 **개념**이 존재하며, 이것은 <u>eval 함수가 **호출**된 Execution Context 를 가리킨다.</u><p>
  
  - <span style="color:#6298c1">eval</span>  함수를 통해, 선언된 **변수**, **함수**는 Calling Context 내부 <span style="color:#c11f1f">VO</span> 에 영향을 준다.(Calling Context 내부 <span style="color:#c11f1f">VO</span> 의 속성으로 할당된다)<p>
         
  - <span style="color:#6298c1">eval</span> 함수로 전달된 **실행 코드**는 생성된 Eval Execution Context 내부에서 처리된다.<p> 
   
  - <span style="color:#c11f1f">VO</span> 는 Eval Execution Context 진입 시 **생성**되며, eval 함수 종료 시 **소멸**된다.
      ...
      - Global Execution Context 에서의 eval 함수
              ...
              // x 속성은 해당 calling context 내부 VO 에 영향을 준다.
              eval('var x = 1');
  ...
  - 단 <span style="color:#c11f1f">strict-mode</span>(**ES5**) 에서의 <span style="color:#6298c1">eval</span> 함수는 Calling Context 내부 <span style="color:#c11f1f">VO</span> 에 영향을 주지 않으며, 코드를 **지역 샌드박스**(local sandbox)에서 평가하게된다.
        ...
        - Global Execution Context 의 eval 함수
            ...
            // strict-mode 에서는 eval 함수를 통해, 선언된 변수는 해당 calling context 에 영향을 주지않으며, local sandbox 안에서 평가된다.
            eval('var y = 2;');
            
            console.log(x); // 1
            
            // calling context 내부 VO 에 영향을 주지 않았다.
            console.log(y); // Uncaught ReferenceError: y is not defined
            ```
        ...
        - <span style="color:#c11f1f">ECStack</span> 내부
            ...
            var ECStack = [
              ...
              globalExecutionContext: {
                VO: {
                  // y 속성이 추가되지 않았다(즉 해당 calling context 내부 VO 에 영향을 주지 않았다)
                  x: 1
                }
              }
            ];
