...
- <span style="color:#c11f1f">VO</span> in Global Execution Context
	...
	- **전역 객체**의 **생성 시점**은 Execution Context(Global, Function, Eval Context) **진입 시점 전**이며, **소멸 시점**은 **프로그램 종료 시점**이 된다.
	
	- **전역 객체**는 <span style="color:#6298c1">this</span> 키워드를 통해 접근가능하다.
...
- <span style="color:#c11f1f">VO</span> in Function Execution Context

	- Function Execution Context 의 <span style="color:#c11f1f">VO</span> 는 <u>직접적인 접근이 불가능하며</u>, 활성화 객체(<span style="color:#c11f1f">AO</span>)가 이 역활을 대신한다.
	...
	- <span style="color:#c11f1f">AO</span> 는 **함수 호출** 시 Function Execution Context 로 들어가면서 **생성**된다.
