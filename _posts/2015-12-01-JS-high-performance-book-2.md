...
- <h3>원글: <em><strong>DOM</strong>이란? XML 문서와 HTML 문서에서 동작하는 언어 독립적인 <span style="color:#c11f1f">API</span>(Application Programing language) 입니다.</em></h3>

  - **DOM** 은 모든 언어(C, C++, Python, JS 등)를 통해 **구현** 가능한 <span style="color:#c11f1f">Programing Interface</span> 를 정의하고 있다.<p />
  
  - 그럼 이 원글에서 말하는 **API** 란? <u>**DOM** + **JS**(또는 다른 모든 언어) = <span style="color:#c11f1f">API</span></u> 라는 방정식으로 나타낼 수 있다.<p />
  
          ```javascript
          
          // 위 방정식에 대한 원문
          
          // https://developer.mozilla.org/ko/docs/Gecko_DOM_Reference/Introduction
          
          In the beginning, JavaScript and the DOM were tightly intertwined, but eventually they evolved into separate entities. 
          The page content is stored in DOM and may be accessed and manipulated via JavaScript, 
  ...
  - 즉 **브라우저 기반**에서의 **DOM** 은, 웹 서버로부터 내려받은 (정적)HTML(or XHTML, XML)의 각 요소(DocType, html, body 등)들을, **JS** 를 통해 **제어** 할 수 있도록 만든, **계층화한 구조체**를 의미한다.
